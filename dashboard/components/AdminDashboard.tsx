...
function useOrganization(filters: { range: string; search: string; repository: string; employee: string }, page: number, pageSize: number) {
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(() => load(false), 15_000);
...
function EmployeeDirectory({ data }: { data: OrganizationSnapshot }) {
  ...
  return <section className="directory-layout"><aside className="employee-list"><header><span className="panel-kicker">Directory</span><strong>{data.employees.length} employees on this page</strong></header>{ordered.map((item) => <button className={item.login === employee.login ? "active" : ""} key={item.login} onClick={() => setSelected(item.login)}><span className="avatar">{item.avatar}</span><span><strong>{item.name}</strong><small>@{item.login} · {item.repositories.length} repos · {item.open} open</small></span><b className={item.score < 60 ? "danger-text" : "good-text"}>{item.score}</b></button>)}</aside><article className="employee-profile"><header><div><span className="panel-kicker">Combined across all repositories</span><h2>{employee.name} <small>@{employee.login}</small></h2></div><span className={`profile-score ${employee.score < 60 ? "risk" : "healthy"}`}><b>{employee.score}</b><small>security score</small></span></header><div className="profile-kpis"><span><small>Repositories</small><b>{employee.repositories.length}</b></span><span><small>Clean-check accuracy</small><b>{employee.cleanRate}%</b></span><span><small>Open errors</small><b>{employee.open}</b></span><span><small>Fix rate</small><b>{employee.fixRate}%</b></span><span><small>Mean time to fix</small><b>{employee.avgFixHours}h</b></span><span><small>Repeat errors</small><b>{employee.repeats}</b></span></div><PersonalProgress employee={employee} data={data}/><div className="profile-columns"><div><h3>Repository-wise work accuracy</h3>{employee.repositories.map((repo) => <div className="repo-contribution" key={repo.repository}><strong>{repo.repository}<small>{repo.checks} PR checks · {repo.blocked} blocked · {repo.errors} errors</small></strong><span className="repo-accuracy"><b>{repo.cleanRate}%</b><i><em style={{ width: `${repo.cleanRate}%` }}/></i><small>clean accuracy</small></span></div>)}</div><div><h3>Recent attributed errors</h3>{incidents.length ? incidents.map((incident) => <div className="profile-incident" key={incident.id}><span className={`severity ${incident.confidence}`}>{incident.confidence}</span><strong>{incident.resource}<small>{incident.repository} · {incident.file}</small></strong><em>PR #{incident.pr}</em></div>) : <p className="empty-state">No recent errors for this employee.</p>}</div></div></article></section>;
}
