...
async function github(path: string): Promise<JsonObject[]> {
  ...
  const response = await fetch(`${API}${path}`, {
    headers: { Accept: "application/vnd.github+json", Authorization: `Bearer ${token}`, "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "LeakGuard-Control-Plane" },
    cache: "no-store",
  });
...
async function performSync(force = false): Promise<SyncResult> {
  ...
  if (!token || !organization) return { repositories: 0, members: 0, runs: 0, skipped: true };
  const connection = db();
  ...
  const repositories = await paginated(`/orgs/${encodeURIComponent(organization)}/repos?type=all&sort=updated`, cap);
  ...
  for (let start = 0; start < repositories.length; start += concurrency) {
    await Promise.all(repositories.slice(start, start + concurrency).map(async (repository) => {
      ...
      const [collaborators, workflowRuns] = await Promise.all([
        github(`/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/collaborators?per_page=100`).catch(() => []),
